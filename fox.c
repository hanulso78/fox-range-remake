...
// https://www.codingnow.com/allegro/alleg001.htm
int stage_number = -1;
...
void launch_guided_missile (int count, BITMAP *bitmap, int power, int speed)
{
	...
	for (i=0; i<count; i++) {
		...
		if ( list_size(enemy_list) > 0 ) {
			if ( list_size(enemy_list)-1 < enemy_target_index ) {
...
void process_enemy_movement(void)
{
	...
	while (list_iter_next(&iter, (void*) &v) != CC_ITER_END) 
	{
		...
		if ( stage_elapsed_time >= e->time ) {
			...
			int j=0;
			for(j=0; j<list_size(res->bitmaps); j++) {
				void *v;
		...
		while (list_iter_next(&iter, (void*) &v) != CC_ITER_END) 
		{
			...
			// 적기 이동 처리
				...
				if ( list_size(enemy->bitmaps) > 1 && strcasecmp(enemy->name, "BOSS") ) {
					...
					if ( diff >= 100 ) {
						if ( enemy->bitmap_index >= list_size(enemy->bitmaps)-1 ) {
							enemy->bitmap_index=0;
						} else {
							enemy->bitmap_index++;
						}
...
void process_fox_guided_missile(void)
{
	...
	while (list_iter_next(&iter, (void*) &v) != CC_ITER_END) 
	{
		...
		if ( !check_collision_bbox(0, 0, SCREEN_W, SCREEN_H, x, y, w, h) ) {
			...
			// 유도탄과 적들의 충돌 체크
			for(j=0; j < list_size(enemy_list); j++) {
				void *v2;
...
void free_fox_weapon(void)
{
	...
	while (hashtable_iter_next(&iter, &entry) != CC_ITER_END) {
		...
		void *out;
		hashtable_get(fox_weapon_table, key, (void*) &out);
		List *list = (List*)out;
...
int free_stage_resource(void)
{
	...
	if ( stage_res_table != NULL ) {
		...
		while (hashtable_iter_next(&iter, &entry) != CC_ITER_END) {
			...
			void *out;
			hashtable_get(stage_res_table, key, (void*) &out);
			ENEMY_RES *res = (ENEMY_RES*)out;
...
int stage_destroy()
{
	...
	if ( orbit_table != NULL ) {
		...
		while (hashtable_iter_next(&iter, &entry) != CC_ITER_END) {
			...
			void *out;
			hashtable_get(orbit_table, key, (void*) &out);
			List *list = (List*)out;
...
int stage_start(int stage)
{
	...
	// -------------------------------------------------------
	show_popup_message("STAGE 1\nOUTER SPACE", TRUE, 3000);
...
void load_kssm_font()
{
	...
	font_data = load_datafile_callback("FONT.DAT", load_callback);
	if (font_data) {
...
#ifdef USE_DATAFILE
			char file_name[1024];
			sprintf(file_name, "STAGE%d.DAT", 1);

			stage_data = load_datafile_callback(file_name, load_callback);
			if ( stage_data == NULL ) {
				err("Couldn't load sprite resources!");
			}
#endif
...
#else
			char tmp[1024];
			sprintf(tmp, "RES/STAGE%d/STAGE.BMP", 1);
			PALETTE pal;
